...
function Head(props) {
  ...
  const menu = (
    <Menu>
      ...
      onCancel={cancel}
      >
      <Link to={"./guide"}>{t('page.logout')}</Link>
      </Popconfirm>
      </Menu.Item>
    </Menu>
  );
  ...
  let userElem = (
    <Link to="/login">
      <span className="login">{t('page.login')}</span>
    </Link>
  );
  ...
  if(location.pathname === "/guide"){
    ...
    elemGuide=(<>
      <span className="link linkGuide1" onClick={clickdo}>
        {t("goal.goal")}
      </span>
      <span className="link linkGuide2"  onClick={clickHave}>
        {t("goal.SAMA")}
      </span>
    </>
    )
